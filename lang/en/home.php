<?php 

 return [

    'anasayfa' => 'Homepage' ,
    'hakkımızda' => 'About us' ,
    'ürünler' => 'Products',
    'iletişim' => 'CONTACT' ,
    'neleryapıyoruz' => 'QUALITY' ,
    'adres' => 'ISMET PASHA NEIGHBORHOOD TRABZON BOULEVARD. NO:29  BİLLUR APT. FLOOR:4 NO:8' ,
    'siparis' => 'For Orders' ,


   'hakkımızda1' => 'ASEM CAM KAPAK company was established in 2005.
   First of all, we have added production to our trade, which we started by importing from Indonesia and China.
    We are currently producing the latest technology fully automatic glass cover.
     Our glass cover production continues as 2 lines.
    Our monthly capacity is 400,000 pieces. For the steel strip around the glass cover, 
    the latest model automatic LASER is welded without welding traces. This adds aesthetics to 
    the glass cover. We can produce C type T type Z type and Teapot glass lids in our facility.
     We can produce glass covers from 8cm glass covers to 36cm numbers. Our facility has the capacity
      to produce according to the wishes of the customers. Having an automatic facility in our facility,
       it is inevitable that the sensitive glasses will have better quality hygiene. ',
   'hakkımızda2' => '',
   'hakkımızda3' => 'According to our customers requests.
    We produce brown, blue and fload flat glass glass covers using 4 mm and 5 mm glasses. ',
   'hakkımızda4' => '',
   'hakkımızda5' => 'Our company has always adopted the policy of producing quality products and providing the best service to its customers with its experienced staff. ',
   'hakkımızda6' => '',
   'hakkımızda7' => 'The quality of our products is passed through quality control and testing stages and shipped to our customers. ',
   'hakkımızda8' => '',
   'hakkımızda9' => 'Our products are A Quality.',
   'hakkımızda10' => '',

   'hakkımızdakısa' => 'ASEM GLASS  company was established in 2005. First of all, we have added production to our trade, which we started by importing from Indonesia and China. We are currently producing the latest technology fully automatic glass cover. Our glass cover production continues as 2 lines. ',

   'faaliyet' => 'TESTS FOR PRODUCT QUALITY',
   // 'activity1' => 'Ball Test',
   // 'activity2' => 'Bomb Control',
   'faaliyet3' => 'Ball Test',
   'faaliyet4' => '',
   'faaliyet5' => 'Bomb Control',
   'faaliyet6' => '',
   'faaliyet7' => 'Optical Distortion',
   'faaliyet8' => '',
   'faaliyet9' => 'Temper Test',
   'faaliyet10' => '',
   'faaliyet11' => 'Withstand High Temperature',
   'faaliyet12' => '',
   'faaliyet13' => '',
   'faaliyet14' => 'Zebra Test',
   'faaliyet15' => 'Quality controls are carried out with the SHOCK COOLING Test tests.',


   'projeler' => 'Products',
   'projeler1' => 'C Type',
   'projeler2' => 'T Type',
   'projeler3' => 'G Type',
   'projeler4' => '',



   'musteriler' => 'Customers',
   'musteriler1' => 'Customers',
   'musteriler2' => 'Customers',
   'musteriler3' => 'Customers',
   'musteriler4' => 'Customers',
   'musteriler5' => 'Customers',
   'musteriler6' => 'Customers',
   'musteriler7' => 'Customers',
   'musteriler8' => 'Customers',


   'çalıştığımız' => 'Our references',
   'çalıştığımız2' => '',
   'aşağı' => 'down',
   'takipedin' => 'Follow',
   
   'gonder' => 'Send' ,
    'dahafazla' => 'More' ,


   'mutlu' => '',
   'mutlu1' => '',
   'mutlu2'=> '',
   'mutlu3' => '',
   'mutlu4' => '',

   'bizeulaşın' => '',
   'bizeulaşın2' => '',
     'email' => 'example@gmail.com',
    'özellikler' => 'FEATURES',
    'markalaşma' => 'BRANDING' ,
    'gelişim' => 'DEVELOPMENT' ,
    'Danışmanlık' => 'Consulting' ,
    'işler' => 'What We Do' ,
    'Poliüretan' => 'Polyurethane Thermal Insulation' ,
    'Polyurea' => 'Polyurea Waterproofing' ,
    'SuYalıtım' => 'Our Waterproofing Applications',
    'Overlay' => 'Overlay Applications' ,
    'Spin' => 'Polyurethane Squeeze Foam' ,
    'Herşey' => 'ALL' ,
    'tercih' => 'BRANDS THAT PREFER US' ,
     'Kalite' => 'Slogan 1',
     'Kalite2' => 'Slogan 2',
     'müşteri' => 'CUSTOMER SATISFACTION',
     'saat' => 'WORKING HOUR',
     'mennun' => 'SATISFIED CUSTOMERS',
     'teslim' => 'DELIVERED PROJECTS',
     'başarılar' => 'ACHIEVEMENT',
   

 ]

?> 